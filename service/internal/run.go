...
import (
	...
	"github.com/liankui/git-contributions-draw/service/pkg/handlers"
	"github.com/liankui/git-contributions-draw/service/pkg/svc"
)

const FullServiceName = "GitContributionsDraw.GitContributionsDrawServer"

var _ nclient.Config

func NewEndpoints(options map[string]interface{}) svc.Endpoints {
	// Business domain.
	var service pb.GitContributionsDrawServer
	{
	...
	// Endpoint domain.
	var (
		getDrawEndpoint = svc.MakeGetDrawEndpoint(service)
	)
...
func RegisterService(cfg nserver.Config, r *mux.Router, s *grpc.Server) svc.Endpoints {
	...
	pb.RegisterGitContributionsDrawServer(s, svc.MakeGRPCServer(endpoints, tracer, logger))
...
func Run(cfg nserver.Config) {
	...
	// Run!
	logs.Info("GitContributionsDraw.GitContributionsDrawServer", " started.")
	logs.Info("GitContributionsDraw.GitContributionsDrawServer", <-errc)
	logs.Info("GitContributionsDraw.GitContributionsDrawServer", " closed.")
}
