...
import (
	...
	// This Service
	"github.com/liankui/git-contributions-draw/service/internal"
)
...
func main() {
	...
	err := config.ScanFrom(&conf, "gitContributionsDrawServer", "server")
	if err != nil {
	...
	if len(gitHash) == 0 {
		logs.Infow("starting GitContributionsDrawServer, you can use `-ldflags \"-X main.gitHash=xxx\" to generate gitHash")
	} else {
		logs.Infow("starting GitContributionsDrawServer", "gitHash", gitHash, "gitBranch", gitBranch, "buildTime", buildTime)
	}
